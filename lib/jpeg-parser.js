...
class JpegParser extends EventEmitter {
    ...
    push(data) {
        ...
        if (this._end >= 0) {
            ...
            if (this._end < this._buffer.length) {
                consume -= this._buffer.length - this._end;
            }
    ...
    _findFragment(offset) {
        ...
        var index = this._buffer.indexOf(0xff, offset);

        if ((index + 1) >= this._buffer.length) {
            return -1;
        }
    ...
    _getFragmentId(offset) {
        if (offset < 0 || (offset + 1) >= this._buffer.length) {
            return -1;
        }
